...
';
	...
	function wpsc_st_manage_purchase_logs_actions_button( $default = null, $column_name = null, $item = null ) {
		...
		if( $column_name == 'actions' ) {
			if( isset( $item->id ) && $item->id ) {
				$output .= '<a href="' . add_query_arg( array( 'c' => 'item_details', 'id' => $item->id ) ) . '" class="button-primary"><abbr title="' . __( 'View Sale', 'wpsc_st' ) . '">' . __( 'View', 'wpsc_st' ) . '</abbr></a>';
			}
		}
		return $output;
	...
	function wpsc_st_right_now_widget() {
		...
		$count_sql = "SELECT COUNT(`id`) FROM `" . $wpdb->prefix . "wpsc_purchase_logs` WHERE processed IN ('1','2')";
			...
			case 'variations':
				...
				if( $variations ) {
					foreach( $variations as $variation ) {
						...
						$wpdb->query( "DELETE FROM `" . $wpdb->terms . "` WHERE `term_id` = " . $variation->term_id );
					}
				}
...
*/
	...
	function wpsc_st_get_session_id( $purchase_id ) {
		...
		$output = '';
		if( $purchase_id ) {
			$session_id_sql = "SELECT `sessionid` FROM `" . $wpdb->prefix . "wpsc_purchase_logs` WHERE `id` = '" . $purchase_id . "' LIMIT 1";
...
function wpsc_st_get_purchase_id( $session_id ) {
	...
	$output = '';
	if( $session_id ) {
		$purchase_id_sql = "SELECT `id` FROM `" . $wpdb->prefix . "wpsc_purchase_logs` WHERE `sessionid` = '" . $session_id . "' LIMIT 1";
